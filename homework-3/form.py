...
class MainWindow(QWidget):
    ...
    def setUpMainWindow(self):
        ...
        header_label = QLabel("Форма запиту")
        ...
        submit_button = QPushButton("ВІДПРАВИТИ")
        ...
        main_form.addRow(QLabel("Коментарі чи повідомлення"))
    ...
    def checkFormInformation(self):
        ...
        if self.first_name_edit.text() == "" or self.last_name_edit.text() == "":
            ...
            filename = f"{self.last_name_edit.text()}-{self.first_name_edit.text()}.txt"
            i = 1
            while os.path.exists(filename):
                i += 1
                filename = f"{self.last_name_edit.text()}-{self.first_name_edit.text()}-{i}.txt"

            with open(filename, "w") as file:
                file.write(f"Ім'я: {self.first_name_edit.text()}\n")
                file.write(f"Прізвище: {self.last_name_edit.text()}\n")
                file.write(f"Стать: {self.gender_combo.currentText()}\n")
                file.write(f"Дата народження: {self.birthdate_edit.date().toString('MM/dd/yyyy')}\n")
                file.write(f"Телефон: {self.phone_edit.text()}\n")
                file.write(f"Email: {self.email_edit.text()}\n")
                file.write(f"Коментарі чи повідомлення: {self.extra_info_tedit.toPlainText()}\n")

            self.feedback_label.setText("[INFO] Form information saved.")
